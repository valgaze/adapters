...
export const description = (input: any) => {
  return {
    type: "description",
    title: input.title,
    text: [input.text],
...
export const dialogflowmessengerAdapter = ({
  ...
  const payload = messages.map((message) => {
    if (message.text && message.text.length) {
      textNodes.push(message.text as string)
    }
